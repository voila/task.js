...
function Task(thunk) {
    ...
    this.parent = null;
    this.children = []; // subtasks tree


    var current = task.Task.current(); 
    if(current !== null){ // spawning withing a task
        this.parent = current;
...
Tp.cancel = function() {
    ...
    // subtasks
    this.children.forEach(function(t){ t.cancel(); }) // error handling (exceptions....)
...
function runScheduledTask(task) {
    ...
    try {
        ...
        if (task.threadState === T_CANCELLED) {
            task.thread.close();                     // what is this close () method ?
            task.result = void 0;
